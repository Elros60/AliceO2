...
Bool_t AliMillePede2::InitDataRecStorage(Bool_t read)
{
  ...
  //
  if (fTreeData) {LOG(info) << "Data Records File is already initialized"; return kFALSE;} 
  //
  ...
  if (!read) { // write mode: cannot use chain
    ...
    if (!fDataRecFile) {LOG(fatal) << Form("Failed to initialize data records file %s",GetDataRecFName()); return kFALSE;}
    ...
    if (fDataRecFName.EndsWith(".root")) ch->AddFile(fDataRecFName);
      ...
      if (!inpf.good()) {LOG(info) << Form("Failed on input records list %s\n",fDataRecFName.Data()); return kFALSE;}
      //
      TString recfName;
    ...
    if (nent<1) { LOG(info) << "Obtained chain is empty"; return kFALSE;}
    fTreeData = ch;
...
Bool_t AliMillePede2::InitConsRecStorage(Bool_t read)
{
  // initialize the buffer for processed measurements records
  //
  if (fConsRecFile) {LOG(info) << "Constraints Records File is already initialized"; return kFALSE;} 
  //
  if (!fRecord) fRecord = new AliMillePedeRecord();
  //
  fConsRecFile = TFile::Open(GetConsRecFName(),read ? "":"recreate");
  if (!fConsRecFile) {LOG(info) << Form("Failed to initialize constraints records file %s",GetConsRecFName()); return kFALSE;}
  //
  LOG(info) << Form("File %s used for constraints records",GetConsRecFName());
  if (read) {
    fTreeConstr = (TTree*)fConsRecFile->Get(GetRecConsTreeName());
    if (!fTreeConstr) {LOG(info) << Form("Did not find constraints records tree in %s",GetConsRecFName()); return kFALSE;}
...
Int_t AliMillePede2::GlobalFit(Double_t *par, Double_t *error, Double_t *pull)
{
  ...
  if (par){
    ...
    for (int i=fNGloParIni;i--;) par[i] = GetFinalParam(i);
  }
  //
...
Int_t AliMillePede2::GlobalFitIteration()
{
  ...
  */
  ...
  for (int i=0; i<fNGloConstraints; i++) {
    ...
    if (fkReGroup) {
      for (int jp=csize;jp--;) {
	int idp = indV[jp];
	if (fkReGroup[idp]<0) LOG(fatal) << Form("Constain is requested for suppressed parameter #%d",indV[jp]);
	indV[jp] = idp;
      }
    }
  ...
  int defoutB = dup(1);
  if (defoutB<0) {LOG(fatal) << "Failed on dup"; exit(1);}
  ...
  */
  ...
  const char* faildumpA = Form("mp2eq_after%d.dat",fIter);
  int defoutA = dup(1);
  if (defoutA<0) {LOG(fatal) << "Failed on dup"; exit(1);}
  int slvDumpA = open(faildumpA, O_RDWR|O_CREAT, 0666);
  if (slvDumpA>=0) {
    dup2(slvDumpA,1);
    printf("Solving%d for %d params\n",fIter,fNGloSize);
  ...
  */
  ...
  if (!fgIsMatGloSparse) {
    ...
    if (fNLagrangeConstraints==0) { // pos-def systems are faster to solve by Cholesky
      ...
      else LOG(info) << "Solution of Global Dense System by Cholesky failed, trying Gaussian Elimiation";
    }
    //
    if (((AliSymMatrix*)fMatCGlo)->SolveSpmInv(fVecBGlo, kTRUE)) return kInvert;
    else LOG(info) << "Solution of Global Dense System by Gaussian Elimination failed, trying iterative methods";
  }
  ...
  if      (fgIterSol == AliMinResSolve::kSolMinRes) 
    ...
    LOG(info) << Form("Undefined Iteritive Solver ID=%d, only %d are defined",fgIterSol,AliMinResSolve::kNSolvers);
  //
  if (!res) {
    ...
    else LOG(info) << "Failed on file open for matrix dumping";
    close(defout);
    return kFailed;
  }
...
Bool_t AliMillePede2::IsRecordAcceptable()
{
  ...
  if (runID!=prevRunID) {
	...
	break;
      ...
      if (!prevAns) LOG(info) << Form("New Run is not in the list to accept: %ld",runID); 
    }
  }
  //
  return prevAns;
