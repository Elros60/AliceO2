...
void misAlign(Double_t xcartmisaligm = 0.01, Double_t xcartmisaligw = 0.0,
              ...
              const std::string &fileName =
                  "/Volumes/JeccSSD2T/jcastill/aligno2wrk/testAlign24alnewS1/"
                  "MCHMisAlignment_LHC24al_Fix110Fix15_new2_T5_ShiftY.root") {

  // create a regular geometry
  // o2::mch::test::createStandaloneGeometry();
  // std::string oldGeoFileName = "o2sim_geometry-aligned_reco.root";
  // std::string oldGeoFileName = "TGeoManager_GeometryAligned.root";
  std::string oldGeoFileName =
      "/Volumes/JeccSSD2T/jcastill/aligno2wrk/"
      "testAlign24alnewS1/"
      "o2sim_geometry_ReAlign_LHC24al_Fix110Fix15_new2_T5.root";

  o2::base::GeometryManager::loadGeometry(oldGeoFileName.c_str());
  if (!gGeoManager) {
  ...
  // The misaligner
  o2::mch::geo::MisAligner aGMA(xcartmisaligm, xcartmisaligw, ycartmisaligm,
                                ycartmisaligw, angmisaligm, angmisaligw);
  // aGMA.SetCartMisAlig(xcartmisaligm, xcartmisaligw, ycartmisaligm,
  // ycartmisaligw); aGMA.SetAngMisAlig(angmisaligm, angmisaligw);
  ...
  // auto transformation =
  // o2::mch::geo::transformationFromTGeoManager(*gGeoManager); auto t =
  // transformation(100); o2::math_utils::Point3D<double> po;
  ...
  if (!ccdbHost.empty()) {
    std::string path =
        objectPath.empty()
            ? o2::base::DetectorNameConf::getAlignmentPath(detMCH)
            : objectPath;
  ...
  if (!fileName.empty()) {
    ...
    algFile.WriteObjectAny(&params, "std::vector<o2::detectors::AlignParam>",
                           "alignment");
    algFile.Close();
  }
  // gGeoManager->Export("o2sim_geometry-aligned_.root");
  // gGeoManager->Export("TGeoManager_GeometryAlignedShiftY.root");
  gGeoManager->Export(
      "/Volumes/JeccSSD2T/jcastill/aligno2wrk/testAlign24alnewS1/"
      "o2sim_geometry_ReAlign_LHC24al_Fix110Fix15_new2_T5_ShiftY.root");
